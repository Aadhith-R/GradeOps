...
def _build_grading_prompt(question: QuestionItem) -> str:
    ...
    rubric_lines = "\n".join(
        ...
        for i, rc in enumerate(question.rubric)          # ← unified field name
    )
    ...
    breakdown_template = json.dumps(
        [
            ...
            for rc in question.rubric                    # ← unified field name
        ],
        indent=4,
    )
...
def grade_question(image_source: Union[str, bytes], question: QuestionItem) -> QuestionResult:
    ...
    if _MOCK_MODE:
        ...
        mock_score = sum(rc.points for rc in question.rubric)  # ← unified field name
